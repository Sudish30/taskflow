...
def test_create_project_whitespace_only_name(auth_client):
    response = auth_client.post("/projects", json={"name": "   "})
    assert response.status_code == 422


def test_create_project_trims_name(auth_client):
    response = auth_client.post("/projects", json={"name": "  Trimmed  "})
    assert response.status_code == 201
...
def test_update_project_whitespace_only_name(auth_client, project):
    response = auth_client.put(
...
def test_update_project_trims_name(auth_client, project):
    response = auth_client.put(
        f"/projects/{project['id']}", json={"name": "  Renamed  "}
    )
    assert response.status_code == 200
