...
def test_create_task_whitespace_only_title(auth_client, project):
    response = create_task(auth_client, project["id"], title="   ")
    assert response.status_code == 422


def test_create_task_trims_title(auth_client, project):
    response = create_task(auth_client, project["id"], title="  Report  ")
    assert response.status_code == 201
...
def test_update_task_whitespace_only_title(auth_client, project):
    created = create_task(auth_client, project["id"]).json()
    response = auth_client.put(
        f"/projects/{project['id']}/tasks/{created['id']}",
        json={"title": "   "},
...
def test_update_task_trims_title(auth_client, project):
    created = create_task(auth_client, project["id"]).json()
    response = auth_client.put(
        f"/projects/{project['id']}/tasks/{created['id']}",
        json={"title": "  Updated  "},
